...
- **Host buffer pool** (v1.9): `/etc/init.d/nss-bufpool` (START=95, just after `qca-nss-drv` at 94) sets `n2hcfg/extra_pbuf_core0=802816` then `n2hcfg/n2h_empty_pool_buf_core0=4096`, matching stock RD03v2. It is an init script rather than another `rc.local` line on purpose: `rc.local` is in `/lib/upgrade/keep.d/base-files-essential`, so a plain `sysupgrade` restores the user's old copy over the image's and the block would silently never run for anyone upgrading. `/etc/init.d` is in no keep list. It also skips the `extra_pbuf` write below 20 MB `MemFree` (not `MemAvailable`, which counts reclaimable memory an atomic allocation can never use), because that allocator is `kzalloc(GFP_ATOMIC)` with a `BUG_ON` on a failed first page of each of its 7 messages, and this board runs `panic_on_oops=1` with `kernel.panic=3` — a failed allocation at boot would be a reboot loop, not a warning. The non-LOW profile default of 8704 empty skbs is pinned in Linux slab as `SUnreclaim`; halving it and buying the descriptors back is worth **~5.5 MB**, measured boot-to-boot on the bench (`SUnreclaim` 44,576 → 38,992 kB, `MemAvailable` 37,088 → 40,016 kB), with `n2h_pbuf_def_total_count` reaching stock's 14,884. `extra_pbuf_core0` is written first because it is **write-once per boot** (`-EPERM` afterwards) and, despite the name, is host memory rather than the `nss@40000000` carve-out. See [`../stock-investigation/notes/V1.9-TUNING.md`](../stock-investigation/notes/V1.9-TUNING.md) finding #1.
