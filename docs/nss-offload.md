...
- **Host buffer pool** (v1.9): `/etc/init.d/nss-bufpool` (START=96 — after `qca-nss-drv` at 94, and after `done`/`rc.local` at 95) sets `n2hcfg/n2h_empty_pool_buf_core0=4096`, stock RD03v2's value, instead of the profile default 8704. Those are empty skbs pinned in Linux slab as `SUnreclaim`, so halving them returns memory directly: **about −6.3 MB** measured on matched idle boots (`SUnreclaim` 44,576 kB without it, 38,204 / 38,144 kB with it over two boots). It is an init script rather than another `rc.local` line because `rc.local` is in `/lib/upgrade/keep.d/base-files-essential` and a plain `sysupgrade` restores the user's old copy over the image's — anything added there would silently never run for an upgrading user. It deliberately does **not** set stock's `extra_pbuf_core0=802816`: that *costs* ~784 kB of host memory (it is `kzalloc(GFP_ATOMIC)` pages, not the `nss@40000000` carve-out), it is write-once per boot, and its allocator calls `BUG_ON` on a failed page across each of 7 messages — which on this board (`panic_on_oops=1`, `kernel.panic=3`) means a boot-time reboot loop. Since `kzalloc(PAGE_SIZE)` refills `kmalloc-4k` at **order 3**, the failure mode is fragmentation, which no `MemFree`/`MemAvailable` pre-check can see. Trading that tail risk for 4,900 NSS descriptors is a bad deal; the pool write allocates nothing and has no such path. See [`../stock-investigation/notes/V1.9-TUNING.md`](../stock-investigation/notes/V1.9-TUNING.md) finding #1.
