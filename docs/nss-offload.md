...
- **Host buffer pool** (v1.9): the same `rc.local` block sets `n2hcfg/extra_pbuf_core0=802816` then `n2hcfg/n2h_empty_pool_buf_core0=4096`, matching stock RD03v2. The non-LOW profile default of 8704 empty skbs is pinned in Linux slab as `SUnreclaim`; halving it and buying the descriptors back is worth **~5.5 MB**, measured boot-to-boot on the bench (`SUnreclaim` 44,576 → 38,992 kB, `MemAvailable` 37,088 → 40,016 kB), with `n2h_pbuf_def_total_count` reaching stock's 14,884. `extra_pbuf_core0` is written first because it is **write-once per boot** (`-EPERM` afterwards) and, despite the name, is host memory rather than the `nss@40000000` carve-out. See [`../stock-investigation/notes/V1.9-TUNING.md`](../stock-investigation/notes/V1.9-TUNING.md) finding #1.
